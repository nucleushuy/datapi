...
test(
	"transformation HTTP binds explicit approval to scoped preview and persists immutable undo redo",
	{ timeout: 180_000 },
	async () => {
		...
		try {
			...
			await seed.upload(
				...
				job.id,
				(async function* () {
					yield Buffer.from("category,value\nA,1\nB,2\nA,3\n");
				})(),
			);
		...
		try {
			...
			const spec: TransformSpec = {
				version: 1,
				datasetVersionId: original.currentVersionId,
			...
			assert.equal(
				(
					await fetch(`${app.url}${prefix}/preview`, {
						method: "POST",
			...
			assert.deepEqual(preview.impact.after, [
				["A", "1"],
				["A", "3"],
			]);
			...
			for (const body of [
				...
				{ previewId: preview.id, approved: true, spec },
			]) {
			...
			assert.equal(
				applied.versions.find((v) => v.id === applied.currentVersionId)?.parentVersionId,
			...
			assert.deepEqual(((await (await request(`${base}/preview`)).json()) as Preview).rows, [
				["A", "1"],
				["A", "3"],
			]);
			...
			assert.equal(
				(await request(`${prefix}/undo`, "POST", { expectedVersionId: original.currentVersionId })).status,
				409,
			);
			await app.close();
			...
			const discardedResponse = await request(`${prefix}/preview`, "POST", {
				spec: { ...spec, datasetVersionId: applied.currentVersionId },
			});
			...
			assert.ok(
				(await request(`${prefix}/apply`, "POST", { previewId: discarded.id, approved: true })).status >= 400,
			);
		} finally {
			await app.close();
